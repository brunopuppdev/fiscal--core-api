...
- **`DB_SYNCHRONIZE=true` por padrão, sem migrations versionadas do TypeORM** — é o item de maior urgência relativa deste roadmap. Diferente das demais limitações (que dependem de carga real ou de definição regulatória para justificar mudança), o custo de introduzir migrations só cresce com o tempo: uma vez que existam dados reais em produção ou mais de um ambiente/desenvolvedor sincronizando o schema, qualquer alteração futura de schema passa a arriscar dado real, o que não acontece hoje enquanto o schema ainda nasce do zero a cada `synchronize`. Como já houve uma emissão real autorizada em produção (05/08/2026, `cStat 100` — ver item acima), essa condição deixou de ser hipotética. Vale gerar a baseline de migrations e desligar `synchronize` antes da próxima mudança de schema, não depois.
